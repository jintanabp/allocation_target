...
| `AGGREGATE_LOAD_WORKERS` | `6` (ไม่ต้องตั้ง — ตั้ง `1` เฉพาะตอนไล่ปัญหาโหลดรวมภาค) |

## การกระจายหีบ — สวิตช์ที่ควรรู้

| ตัวแปร | ค่าแนะนำ | ผลถ้าตั้งผิด |
|--------|----------|--------------|
| `ALLOC_ALLOW_MISMATCH` | **ไม่ตั้ง** (ปิด) | ตั้ง `1` = ปล่อยผลกระจายที่ผลรวมต่อ SKU ไม่ตรงเป้าให้บันทึกและส่งต่อได้ ใช้เฉพาะกู้สถานการณ์แล้วปิดกลับทันที |
| `ALLOC_ZERO_BASELINE_CAP` | **ไม่ตั้ง** (ปิด) | ตั้ง `1` = จำกัดหีบของคนที่ไม่มีประวัติขาย SKU นั้น **ตัวเลขจะย้ายที่ราว 2–3.5% ต่อทีม** ต้องแจ้งผู้ใช้ก่อนเปิด |
| `ALLOC_REQUIRE_IF_MATCH` | ตั้ง `1` ได้หลังยืนยัน rollout | บังคับ client ส่ง version ตอนบันทึก กัน tab เก่าเขียนทับ — ดูขั้นตอนเปิดใน `docs/CONCURRENCY.md` |

> `ALLOC_REQUIRE_IF_MATCH=1` เคยทำให้ปุ่ม **ส่งเข้า Target Sun** พังทุกครั้ง (แก้แล้ว)
> ถ้า deploy จาก build เก่ากว่านี้ อย่าเพิ่งเปิด
