...
คู่พนักงาน×สินค้าที่**ไม่เคยขายกันเลยใน 12 เดือนล่าสุด**จะไม่ได้รับเป้า (หีบไปอยู่กับคนที่เคยขายจริงแทน) — มีข้อยกเว้น 2 แบบที่**เฉลี่ยให้ทุกคน**แทนการตัด: (1) ทั้งทีมไม่มีใครเคยขาย SKU นั้นเลย (2) เป้าใหญ่เกิน `push_multiple` เท่า (ค่าเริ่มต้น 5) ของประวัติรวม — กันสินค้าที่เพิ่งถูกดันเป้าไปกองที่คนเดียว
...
- **`force_min_one` ("ทุกคนอย่างน้อย 1 หีบ") ชนะกติกานี้เสมอ** — เจตนาผู้ใช้ชัดกว่ากติกาอัตโนมัติ
...
> เขียนไฟล์รอบเดียวใต้ lock เดียว — ต่างจากการติ๊กทีละคนที่อ่าน+เขียนไฟล์ทั้งไฟล์ทุกครั้ง
> (ดู `docs/CONCURRENCY.md` หัวข้อ lost update ของ `user_access.json`)
