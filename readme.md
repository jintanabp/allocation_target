...
[Step 4] ตรวจสอบผล / แก้หีบด้วยมือ (ระบบเกลี่ยส่วนต่างให้อัตโนมัติ)
        ↓
[Export] **ส่งเข้าระบบเป้า TargetSun** (multipart Excel ผ่าน API SPC) หรือ **ดาวน์โหลด** Excel TGA / ไฟล์สรุป Dashboard
```
...
- **Grain เดียวกับ semantic:** ประกอบด้วย **AREACODE / PROVINCECODE / DIVISIONCODE / SALESTYPE** ตามมุมมองเป้ารายเม็ดจาก Fabric เชื่อมกับ **พนักงาน × SKU** ที่กระจายแล้ว
- **`tga_lines` cache:** เมื่อดึงพนักงาน ระบบพยายามเขียนไฟล์เส้นสายเป้ารายเม็ดลง **`data/tga_lines_{SuperCode}_{year}_{month}.csv`** เพื่อใช้ผสานกับหีบและแท็ก grain อย่างสม่ำเสมอเมื่อมีหลายแถวในชั้นข้อมูลเดียวกัน (**รักษารหัส AREACODE ให้เหมือน Fabric รวมศูนย์นำหน้า เช่น `0` เพื่อ round-trip**)

### Integration: ส่งผลเข้า TargetSun (SPC)

- จากหน้า Dashboard โหมด **ส่งเข้าระบบเป้า** จะเรียก **`POST /lakehouse/import-targetsun`** — backend ประกอบ Excel ในรูปแบบ `importTargetSalesmanNextFromExcel` แล้วยิง **HTTPS multipart** ไปยัง URL ใน **`TARGETSUN_IMPORT_EXCEL_URL`** (ค่าเริ่มต้น UAT — เปลี่ยนได้ใน `config/.env`)
- **แอปนี้ไม่เชื่อม Oracle โดยตรง** (เช่น host ภายใน SPC เช่น `10.109.9.x` ) — เขียนเข้าฐานอยู่ฝั่งบริการของ SPC หลัง import สำเร็จ
- รายการชื่อคอลัมน์, ตัวอย่างเรียก, และค่าเริ่มต้น UAT: โปรเจ็กต์มีเอกสาร **`targetsun-importTargetSalesmanNextFromExcel.md`**

ถ้ารับ **502 Bad Gateway**, **timeout**, หรือ **SSL handshake** — ใน response จาก API (และ toast ใน UI) มักมีข้อความช่วยเหลือภาษาไทย (`hint_th`): เปิดจาก **การ์ดข้อความข้อผิดพลาด** ใน modal ได้ (ส่วนสำหรับ IT)
...
| `POST` | `/lakehouse/import-targetsun` | ส่ง Excel เดียวกันไป **`importTargetSalesmanNextFromExcel`** (multipart) |
...
| ส่งเข้า TargetSun แล้ว **502 / Timeout / SSL error** | โปรกซีเกตเวย์ระหว่างแอปกับโฮสต์ SPC / การยืนยันใบรับรอง / เครือข่าย | เช็ค `TARGETSUN_IMPORT_EXCEL_URL`, `TARGETSUN_IMPORT_VERIFY_SSL`, `TARGETSUN_IMPORT_TIMEOUT_SEC` และความเข้าถึงจากเครื่องรันแอป; อ่าน `detail.message` และ `hint_th` จาก `POST /lakehouse/import-targetsun` |
