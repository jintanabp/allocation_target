...
**Dim_Salesman** ใช้เฉพาะดึงพนักงานใต้ `SuperCode` และ validate (`scripts/validate_access_with_dim.py`)  
...
**อัปเดตสิทธิ์:** `python scripts/import_user_access_from_division_xlsx.py` → `rebuild_access_hierarchy.py` → รีสตาร์ท server (ดู `config/README.md`)
...
- หลัง import/rebuild Excel ต้อง **รีสตาร์ท server** หรือรัน `scripts/rebuild_access_hierarchy.py`
