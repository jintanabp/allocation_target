...
### ล็อกอิน Microsoft (Entra) และสิทธิ์จาก Excel roster
...
ลำดับชั้น Manager → Supervisor คำนวณจากกฎใน roster แล้วเขียนลง **`config/access_hierarchy.json`** (ไม่ใช้ `trf_select_supervisor` / `ACC_USER_CONTROL` ใน runtime)  
...
| `POST` | `/lakehouse/prepare-targetsun` | ขั้นที่ 1 ส่ง Target Sun — สร้าง Excel เก็บชั่วคราว คืน `prepare_token` |
| `POST` | `/lakehouse/import-targetsun` | ขั้นที่ 2 (มี `prepare_token`) หรือสร้าง+ส่งครั้งเดียว — ไป **`importTargetSalesmanNextFromExcel`** |
