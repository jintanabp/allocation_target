...
def read_product_info_df(
    ...
    #
    # แต่ตอนถอยไปใช้ของเก่า (allow_stale) ต้องยอมรับ เพราะถ้าปฏิเสธด้วยจะไม่เหลือ
    # อะไรให้ใช้เลยตอน Fabric ล่ม → ราคาเป็น 0 ทั้งระบบ → ทุกทีมเปิดงวดไม่ได้
    # เหมือนเหตุการณ์ 2026-08-25 · ราคาเครดิตกับทีมรถเงินสดยังใกล้ความจริงกว่า 0 มาก
    if "cash_unit_price" not in rows[0]:
        if not allow_stale:
            ...
            return None
        logger.warning(
            "ใช้แคชสินค้ารุ่นเก่า %d-%02d ที่ยังไม่มีราคารถเงินสด — ดึงของใหม่ไม่ได้ "
            "ทีมรถเงินสดจะได้ราคาเครดิตไปก่อน",
            year, month,
        )
