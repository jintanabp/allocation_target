...
def acknowledge_logs(entry_ids: list[str]) -> int:
    """ลบรายการที่แอดมินรับทราบแล้วออกจากไฟล์ log"""
    want = {str(x).strip() for x in entry_ids if str(x).strip()}
    if not want:
        return 0
    removed = 0
    d = logs_dir()
    ...
    with _LOCK:
        for fn in os.listdir(d):
            if not fn.startswith("usage_") or not fn.endswith(".jsonl"):
                continue
            ...
            try:
                ...
                logger.warning("usage log ack %s: %s", path, e)
    return removed
