...
_CACHE_PREFIXES = (
    ...
    "hist_cy_",
    "emp_cache_",
    # payload_cache_*.json มี TTL ของตัวเองอยู่แล้ว (EMPLOYEE_PAYLOAD_CACHE_TTL_SEC
    # ค่าเริ่มต้น 1 ชม.) แต่ TTL แค่ทำให้ "ไม่ถูกใช้" ไฟล์ยังกองอยู่บนดิสก์ตลอดไป
    # ของที่หมดอายุมาแล้วเป็นวัน ๆ ไม่มีทางถูกใช้อีก — เก็บกวาดได้ปลอดภัย
    "payload_cache_",
)

# ตั้งใจไม่ใส่ target_boxes_ / target_sun_ / tga_lines_ ไว้ในลิสต์ข้างบน
#
# ไฟล์เป้าราย sup ไม่ใช่แค่ cache เร่งความเร็ว แต่เป็น "หลักฐานอ้างอิง" ที่ประตู
# ตรวจก่อนส่งใช้เทียบว่ายอดที่จะส่งตรงเป้าไหม ถ้ามันถูกล้างตามอายุ (เดิม 7 วัน
# และ cleanup ทำงานทุกครั้งที่ start) ทีมที่กลับมาเปิด snapshot เก่ามาส่งจะเจอ
# สภาพ "ไม่มีอะไรให้ตรวจ" ซึ่งตอนนี้บล็อกการส่งด้วย send_target_unverifiable
# — ลบทิ้งจึงเท่ากับสร้างงานให้ผู้ใช้โดยไม่ได้อะไรกลับมา ไฟล์เล็กมากอยู่แล้ว
#
# tga_lines_ (TGA grain ราย emp×sku) ก็อยู่ในกลุ่มเดียวกัน: เส้นทางส่งใช้มัน
# แตกหีบลงคีย์จริงของ Oracle ถ้าไฟล์หาย SKU นั้นจะถูกตัดทั้งตัวตามนโยบาย
# S3.5 แล้วผู้ใช้ต้องไปเกลี่ยมือเองทั้งที่ไม่มีอะไรผิด
