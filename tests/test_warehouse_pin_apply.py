...
class WarehousePinApplyTest(unittest.TestCase):
    ...
    def _write_target_boxes(self, rows):
        pd.DataFrame(rows).to_csv(f"data/target_boxes_{SUP}_2026_10.csv", index=False)
    ...
    def test_no_rules_is_a_no_op(self):
        self._write_target_boxes([{"sku": "SKU1", "section": "702", "supervisor_target_boxes": 10, "price_per_box": 1}])
    ...
    def test_pinned_warehouse_never_seen_before_creates_a_new_leg(self):
        ...
        self._write_target_boxes([{"sku": "SKU1", "section": "702", "supervisor_target_boxes": 5, "price_per_box": 1}])
        self._rule(section="702", area="3", div="S", wh="G999")
    ...
    def test_non_matching_combo_is_left_untouched(self):
        """กติกาผูก section 702/area 3/div S — แถวนี้ section ตรงแต่ area ไม่ตรง ต้องไม่ถูกแตะ"""
        self._write_target_boxes([{"sku": "SKU1", "section": "702", "supervisor_target_boxes": 10, "price_per_box": 1}])
        self._rule(section="702", area="3", div="S", wh="G010")
    ...
    def test_all_zero_group_creates_no_pointless_empty_row(self):
        ...
        self._write_target_boxes([{"sku": "SKU1", "section": "702", "supervisor_target_boxes": 0, "price_per_box": 1}])
        self._rule(section="702", area="3", div="S", wh="G010")
    ...
    def test_same_emp_sku_two_area_division_legs_only_matched_leg_is_touched(self):
        ...
        self._write_target_boxes([{"sku": "SKU1", "section": "702", "supervisor_target_boxes": 20, "price_per_box": 1}])
        self._rule(section="702", area="3", div="S", wh="G010")
        df = pd.DataFrame(
            [
    ...
    def test_missing_section_map_fails_open_without_throwing(self):
        """ไม่มีไฟล์เป้าราย sup เลย (หรือไม่มีคอลัมน์ section) — ต้องข้ามกติกาเงียบ ๆ ไม่ throw"""
        self._rule(section="702", area="3", div="S", wh="G010")
        df = pd.DataFrame([_row("E1", "SKU1", 6, "R082"), _row("E1", "SKU1", 4, "G002")])
        out, stats = lh._apply_warehouse_pin_rules(df, SUP, 10, 2026)  # ไม่ได้เขียน target_boxes csv เลย
        self.assertEqual(stats["matched_groups"], 0)
        pd.testing.assert_frame_equal(out.reset_index(drop=True), df.reset_index(drop=True))
